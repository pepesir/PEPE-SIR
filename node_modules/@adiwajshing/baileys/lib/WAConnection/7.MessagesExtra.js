...
class WAConnection extends _6_MessagesSend_1.WAConnection {
    ...
    async loadMessages(jid, count, cursor, mostRecentFirst = true) {
        ...
        const cursorValue = hasCursor && (chat === null || chat === void 0 ? void 0 : chat.messages.get((0, Utils_1.GET_MESSAGE_ID)(cursor)));
        let messages;
    ...
    async messagesReceivedAfter(date, onlyUnrespondedMessages = false) {
        const stamp = (0, Utils_1.unixTimestampSeconds)(date);
        ...
        await Promise.all(chats.map(async (chat) => {
            await this.findMessage(chat.jid, 5, m => {
                if ((0, Utils_1.toNumber)(m.messageTimestamp) < stamp || (onlyUnrespondedMessages && m.key.fromMe))
                    return true;
                messages.push(m);
            });
        }));
    ...
    async loadMessage(jid, id) {
        ...
        if (chat) {
            ...
            message = chat.messages.get((0, Utils_1.GET_MESSAGE_ID)({ id, fromMe: true })) || chat.messages.get((0, Utils_1.GET_MESSAGE_ID)({ id, fromMe: false }));
        }
        if (!message) {
    ...
    async clearMessage(messageKey) {
        ...
        const chat = this.chats.get((0, Utils_1.whatsappID)(messageKey.remoteJid));
        if (chat) {
            const value = chat.messages.get((0, Utils_1.GET_MESSAGE_ID)(messageKey));
    ...
    async starMessage(messageKey, type = 'star') {
        ...
        if (result.status == 200 && chat) {
            const message = chat.messages.get((0, Utils_1.GET_MESSAGE_ID)(messageKey));
            if (message) {
                message.starred = type === 'star';
                const chatUpdate = { jid: messageKey.remoteJid, messages: (0, Utils_1.newMessagesDB)([message]) };
    ...
    async modifyChat(jid, type, arg) {
        ...
        switch (type) {
            ...
            case Constants_1.ChatModification.mute:
                const strStamp = ((0, Utils_1.unixTimestampSeconds)() + Math.floor(durationMs / 1000)).toString();
                chatAttrs.type = type;
    ...
    async getChatIndex(jid) {
        ...
        if ((0, Utils_1.isGroupID)(jid)) {
            chatAttrs.participant = msg.key.fromMe ? (_a = this.user) === null || _a === void 0 ? void 0 : _a.jid : (0, Utils_1.whatsappID)(msg.participant || msg.key.participant);
        }
        return chatAttrs;
    }
}
...
__decorate([
    (0, Mutex_1.Mutex)((jid, messageID) => jid + messageID)

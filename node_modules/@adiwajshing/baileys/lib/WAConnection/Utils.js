...
exports.Browsers = {
    ...
    appropriate: browser => [platformMap[(0, os_1.platform)()] || 'Ubuntu', browser, (0, os_1.release)()]
};
...
exports.waMessageKey = {
    key: (m) => (5000 + (m['epoch'] || 0)).toString(16).padStart(6, '0') + (0, exports.toNumber)(m.messageTimestamp).toString(16).padStart(8, '0'),
    compare: (k1, k2) => k1.localeCompare(k2)
};
const WA_MESSAGE_ID = (m) => (0, exports.GET_MESSAGE_ID)(m.key);
...
const newMessagesDB = (messages = []) => {
    ...
    messages.forEach(m => !db.get((0, exports.WA_MESSAGE_ID)(m)) && db.insert(m));
    return db;
};
...
function hkdf(buffer, expandedLength, info = null) {
    return (0, futoin_hkdf_1.default)(buffer, expandedLength, { salt: Buffer.alloc(32), info: info, hash: 'SHA-256' });
}
exports.hkdf = hkdf;
...
const delayCancellable = (ms) => {
    ...
    const cancel = () => {
        ...
        reject((0, Constants_1.CancelledError)(stack));
    };
    return { delay, cancel };
};
...
async function promiseTimeout(ms, promise) {
    ...
    const p = new Promise((resolve, reject) => {
        delay
            .then(() => reject((0, Constants_1.TimedOutError)(stack)))
...
const ProxyAgent = (host) => (0, https_proxy_agent_1.default)(host);
...
async function generateThumbnail(file, mediaType, info) {
    if ('thumbnail' in info) {
        ...
        const imgFilename = (0, path_1.join)((0, os_1.tmpdir)(), generateMessageID() + '.jpg');
        try {
...
const encryptedStream = async (media, mediaType, saveOriginalFileIfRequired = true) => {
    ...
    // random name
    const encBodyPath = (0, path_1.join)((0, os_1.tmpdir)(), mediaType + generateMessageID() + '.enc');
    ...
    if (type === 'file') {
        ...
        bodyPath = (0, path_1.join)((0, os_1.tmpdir)(), mediaType + generateMessageID());
...
async function decryptMediaMessageBuffer(message) {
    ...
    */
    ...
    const fetched = await (0, exports.getGotStream)(messageContent.url, {
